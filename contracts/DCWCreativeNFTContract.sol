...
error DCWCreativeNFTContract__MaxSupplyReached();
error DCWCreativeNFTContract__ValueNotEqualPrice();
error DCWCreativeNFTContract__WrongAvenueForThisTransaction();

/// @custom:security-contact dee.woah@gmail.com
contract DCWCreativeNFTContract is 
    ERC721, 
    ERC721Enumerable, 
    ERC721URIStorage, 
    ERC721Burnable,
    ...
    ReentrancyGuard 
{
    ...
    constructor(        
        ...
        ) ERC721("DCWCreativeNFTContract", "DCW") Ownable(msg.sender) {
    ...
    receive() external payable {
        revert DCWCreativeNFTContract__WrongAvenueForThisTransaction();
    }
   ...
   function mintTo(
        ...
        if (tokenId >= i_max_tokens) {
            revert DCWCreativeNFTContract__MaxSupplyReached();
        }
        // make sure there us the right amount of money
        if (msg.value != i_mint_price) {
            revert DCWCreativeNFTContract__ValueNotEqualPrice();
        }
        _tokenIdCounter++;
