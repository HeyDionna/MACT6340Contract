...
async function main() {
  ...
  const DCWCreativeNFTContractFactory = await ethers.getContractFactory(
    "DCWCreativeNFTContract"
  );
  // deploy
  const DCWCreativeNFTContract = await DCWCreativeNFTContractFactory.deploy(
    args.mint_price,
    args.max_tokens,
    args.base_uri,
    args.royaltyArtist,
  ...
  await DCWCreativeNFTContract.deploymentTransaction().wait(15);
  ...
  for (let i = 0; i < 3; i++) {
    const transactionResponse = await DCWCreativeNFTContract.mintTo(ipfs[i], {
